...
for idx, sub in enumerate(subjects_both):
    ...
    list_dpfstar.append(dpfstar)
    list_subject.append(np.repeat(sub, nbv))
    list_volume.append(vol)


fig,ax = plt.subplots()
ax.boxplot(list_dpfstar, positions=list_volume,showfliers=False)
ax.set_xticklabels(np.round(np.array(list_volume)/1000),rotation = 45)
plt.tight_layout()
plt.show()
