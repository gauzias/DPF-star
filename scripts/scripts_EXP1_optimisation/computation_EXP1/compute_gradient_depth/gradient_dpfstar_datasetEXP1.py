...
this script compute the gradient of the dpfstar with the slam package for all the subject of the dataset EXP1
"""
...
for idx, sub in enumerate(subjects):
    ...
    # load alpha
    alphas_path = os.path.join(wd, 'data_EXP1/result_EXP1/depth', sub + '_' + ses, 'dpfstar_surface','alpha_2.csv')
    alphas = pd.read_csv(alphas_path)
    alphas = alphas['alphas'].values
    print(alphas)
    # load
    depth_path = os.path.join(wd, 'data_EXP1/result_EXP1/depth', sub + '_' + ses, 'dpfstar_surface', sub + '_' + ses + '_dpfstar_2.gii')
    ...
    # save
    save_folder = os.path.join(wd, 'data_EXP1/result_EXP1/depth',sub + '_' + ses , 'dpfstar_surface/derivative')
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    df_grad.to_csv(os.path.join(save_folder, sub + '_' + ses + '_grad_dpfstar_2.csv'), index=False)
