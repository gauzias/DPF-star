...
def plot_violin(df, metric, alpha_opt):
    ...
    for a in order:
        if a == alpha_opt:
            pvals[a] = -1  # optimal
        elif len(data_per_alpha[a]) == len(df_opt):
            try:
                test = pg.wilcoxon(data_per_alpha[a], df_opt)
                pvals[a] = test['p-val'].values[0]
            except:
                pvals[a] = 1.0
        else:
            pvals[a] = 1.0

    colors = [pval_to_color(pvals[a]) if pvals[a] != -1 else 'red' for a in order]
    ...
    for i, a in enumerate(order):
        if pvals[a] not in [-1, 1.0]:
            star = pval_to_star(pvals[a])
            if star:
