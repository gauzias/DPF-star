...
list_dpf_star = list()
list_subject = list()
for idx, sub in enumerate(subjects_both[:10]):
    print(sub)
    ...
    list_dpf_star.append(dpfstar)
    list_subject.append(np.repeat(sub, len(dpfstar)))

list_dpf_star = [item for sublist in list_dpf_star for item in sublist]
list_subject = [item for sublist in list_subject for item in sublist]

df = pd.DataFrame(dict(sub = list_subject,
                       depth = list_dpf_star))

fig, ax = plt.subplots()
g = sns.FacetGrid(df, row="sub", hue="sub")
# Draw the densities in a few steps
g.map(sns.kdeplot, "depth",
      bw_adjust=.5, clip_on=False,
      fill=True, alpha=1, linewidth=1.5)
g.map(sns.kdeplot, "depth", clip_on=False, color="w", lw=2, bw_adjust=.5)
