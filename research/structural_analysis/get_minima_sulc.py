...
save_csv_folder = os.path.join(dir_result, "minima_count_sulc")
save_smooth_folder = os.path.join(dir_result, "sulc_smooth")
    ...
    try:
        for hemi in hemis:
            ...
            for session_path in sessions:
                ...
                path_sulc = os.path.join(anat_path, f"sub-{subject_id}_ses-{session_id}_hemi-{hemi}_sulc.shape.gii")

                if not (surface_file and os.path.exists(path_sulc) and label_file):
                    continue

                mesh = rw.load_mesh(surface_file[0])
                sulc = rw.read_gii_file(path_sulc)

                sulc_smoothed = sdg.gaussian_smoothing(mesh, sulc, fwhm=10)

                smooth_hemi_dir = os.path.join(save_smooth_folder, f"hemi_{hemi}")
                os.makedirs(smooth_hemi_dir, exist_ok=True)
                smooth_file = f"sub-{subject_id}_ses-{session_id}_hemi-{hemi}_sulc_smooth.gii"
                rw.write_texture(stex.TextureND(darray=sulc_smoothed), os.path.join(smooth_hemi_dir, smooth_file))

                roi_mask, roi_labels = extract_roi_data(tsv_file, label_file[0])
                minima = get_minima(mesh, sulc_smoothed)
...
def generate_plots(df_summary):
    ...
    fig1_path = os.path.join(save_csv_folder, "figure_sulc_minima_regression.png")
    ...
    fig2_path = os.path.join(save_csv_folder, "figure_sulc_minima_scatter.png")
...
def main(data_root):
    ...
    # Sauvegarde CSV
    summary_csv = os.path.join(save_csv_folder, "sulc_summary.csv")
