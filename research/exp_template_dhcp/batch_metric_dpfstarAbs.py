...
def main():
    ...
    for metric in dpf_metrics:
        for roi in roi_list:
            ...
            ax.set_title(f"{metric} vs Sulc_Mean - ROI: {roi}", fontsize=12)

            # DPFstarAbs
            sns.scatterplot(data=sub_df[sub_df["Hemi"] == "left"],
                            x="Week", y=metric, ax=ax,
                            color='navy', label=f"Left - {metric}", alpha=0.6)

            sns.scatterplot(data=sub_df[sub_df["Hemi"] == "right"],
                            x="Week", y=metric, ax=ax,
                            color='darkred', label=f"Right - {metric}", alpha=0.6)

            # Sulc (comparatif unique, on garde Sulc_Mean)
            if "Sulc_Mean" in sub_df.columns:
                sns.scatterplot(data=sub_df[sub_df["Hemi"] == "left"],
            ...
            fname = f"{roi.replace(' ', '_')}_{metric}"
            output_fig = os.path.join(output_dir, f"{metric}")
            os.makedirs(output_fig, exist_ok=True)
                                      
            fig.savefig(os.path.join(output_fig, f"{fname}_sulc_vs_dpfstarAbs.png"), dpi=300)
            plt.close()
