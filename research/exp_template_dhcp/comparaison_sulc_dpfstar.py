...
def load_combined_data(dpf_folder, sulc_folder):
    df_dpf = load_metric_data(dpf_folder, "DPFstar")
...
def main():
    ...
    # Dossiers
    dpf_folder = r"E:/research_dpfstar/result_dhcpSym/cortical_metrics_dpfstar"
    sulc_folder = r"E:/research_dpfstar/result_dhcpSym/cortical_metrics_sulc"
    output_dir = os.path.join(dpf_folder, "figures_sulc_dpfstar_combined")
    ...
    for roi in roi_list:
        ...
        ax.set_title(f"Comparaison Sulc / DPFstar - ROI: {roi}", fontsize=12)

        # DPFstar
        sns.scatterplot(data=sub_df[sub_df["Hemi"] == "left"],
                        x="Week", y="DPFstar_Mean", ax=ax,
                        color='navy', label="Left - DPFstar", alpha=0.6)

        sns.scatterplot(data=sub_df[sub_df["Hemi"] == "right"],
                        x="Week", y="DPFstar_Mean", ax=ax,
                        color='darkred', label="Right - DPFstar", alpha=0.6)
        ...
        # Sauvegarde
        fname = roi.replace(" ", "_")
        fig.savefig(os.path.join(output_dir, f"{fname}_sulc_vs_dpfstar.png"), dpi=300)
        plt.close()
