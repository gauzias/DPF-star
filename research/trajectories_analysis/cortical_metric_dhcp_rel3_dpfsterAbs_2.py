...
def process_subject(subject_path, info_df, hemi, abr_hemi, save_csv_folder):
    ...
    for session_path in sessions:
        ...
        # compute dpfstar Abs
        dpfstar_values = dpfstar_values/100 * np.power(volume,1/3)
        ...
        for label, roi_name in roi_labels.items():
            ...
            all_session_data.append({
                ...
                'Label': label,
                'DPFstarAbs_Mean_abs': np.mean(np.abs(roi_dpfstar)),
                'DPFstarAbs_Mean': np.mean(roi_dpfstar),
                'DPFstarAbs_Median': np.median(roi_dpfstar),
                'DPFstarAbs_Min': np.min(roi_dpfstar),
                'DPFstarAbs_NegCurv_Mean': np.mean(roi_dpfstar_neg_curv) if roi_dpfstar_neg_curv.size > 0 else np.nan,
                'DPFstarAbs_NegCurv_Median': np.median(roi_dpfstar_neg_curv) if roi_dpfstar_neg_curv.size > 0 else np.nan,
                'DPFstarAbs_NegCurv_Min': np.min(roi_dpfstar_neg_curv) if roi_dpfstar_neg_curv.size > 0 else np.nan,
                'scan_age': scan_age,
                'volume': volume,
                'GI': GI,
                'surface': surface
...
def main(data_root):
    ...
    for hemi, abr_hemi in hemispheres:
        ...
        save_csv_folder = os.path.join(dir_result, "cortical_metrics_dpfstarAbs", f'hemi_{hemi}')
